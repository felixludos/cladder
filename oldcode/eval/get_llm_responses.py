...
enable_pdb = False
if enable_pdb:
    openai_key_alias = 'OPENAI_API_KEY_Klei'
...
root_path = '../../../2304_caubench/causalbenchmark/'
# root_path = '../../temp/cg_latest/'
...
class DataFile:
    ...
    def paraphrase(self, datum):
        ...
        datum['given_info'] = [datum['given_info'], datum['given_info_para']][paraphrase_i % 2]
    ...
    def get_raw_data(self, file, inspect_data=False, exclude_query_types={}, shuffle=True):
        ...
        for datum in data:
            ...
            datum['meta']['query'] = {'query_type': datum['meta']['query_id'],
...
class Scorer:
    ...
    def truth_pred_scorer(self, df):
        df.drop(['prompt', 'question_id', 'desc_id'], axis=1, inplace=True)
...
You are an expert in causal inference. The following question is not a typical commonsense query, but rather a meticulously designed question created by a professor specializing in causal inference, intended to assess the students' mastery of the course content.
        ...
        for model_version, enable_cot, para_i in combs:
            ...
            write_out_file = \
                f'{root_path}/outputs/{data_name}_{model_version}{cot_suffix}{ask_about_suffix}{missing_step_suffix}{para_suffix}.csv'
